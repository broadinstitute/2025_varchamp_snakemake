...
def plot_allele_cell(pm, variant, sel_channel, batch_profile_dict, auroc_df, plate_img_qc, site="05", ref_well=[], var_well=[], max_intensity=0.97, display=False, imgs_dir=TIFF_IMGS_DIR, output_dir=""):
...
def plot_allele_cell_by_id(pm, variant, sel_channel, batch_profile_dict, auroc_df, plate_img_qc, compartment="Cells", dim=128, cell_ids=[], ref_well=[], var_well=[], max_intensity=0.97, display=False, imgs_dir=TIFF_IMGS_DIR, output_dir=""):
...
def plot_allele_cell_single_plate(pm, variant, sel_channel, batch_profile_dict, plate_img_qc, auroc_df=None, site="05", ref_well=[], var_well=[], max_intensity=0.97, display=False, imgs_dir=TIFF_IMGS_DIR, output_dir=""):
            ...
            for var_well in var_wells:
                if cell_ids:
                    plot_allele_cell_by_id(allele_meta_df_dict[bio_rep],
                                           ...
                                           cell_ids=cell_ids,
                                           ref_well=[ref_well],
                                           var_well=[var_well], max_intensity=0.97,
                                           display=display,
                                           imgs_dir=TIFF_IMGS_DIR,
                                           output_dir=output_dir)
                else:
...
def plot_allele_separate_plot(pm, variant, sel_channel, plate_img_qc, auroc_df=None, site="05", ref_well=[], var_well=[], max_intensity=0.97, display=False, imgs_dir="", output_dir=""):
    assert imgs_dir != "", "Image directory has to be input!"
    plt.clf()
    cmap = channel_to_cmap(sel_channel)
    channel = channel_dict[sel_channel]
